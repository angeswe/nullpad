...
For that use case only, an opt-in compose override runs a Caddy sidecar that terminates TLS with a self-signed certificate generated by Caddy's internal CA. While the override is active, nullpad's plaintext port (3015) is unpublished so the app is only reachable through Caddy on 3016:

```bash
docker compose -f docker-compose.yml -f docker-compose.https.yml up -d

# Visit https://<host-ip>:3016 — accept the browser warning on first visit.
```

> **Warning:** `docker-compose.https.yml` is for local development and LAN testing only. Do **not** use it in production or on any publicly reachable host. The self-signed certificate is not trusted by browsers, there is no HSTS preload, and the configuration does not harden Caddy beyond defaults. For production, terminate TLS with a properly managed certificate (Let's Encrypt via Caddy/nginx/Traefik with a real hostname).
